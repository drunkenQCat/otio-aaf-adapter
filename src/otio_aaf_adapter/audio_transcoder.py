...
def _normalize_file_url(url: str) -> str:
    """Strip file:// prefix and normalize Windows drive paths."""
...
def get_wav_format(wav_path: str) -> Optional[Dict]:
    ...
    Returns:
        Dict with 'sample_rate', 'sample_width', 'channels', or
        None if error.
    """
    try:
...
def transcode_audio_files(
    ...
    for audio_path in audio_paths:
        ...
        if os.path.exists(converted_path):
            print(
                f"  Using existing converted file: "
        ...
        if not needs_transcoding(
            wav_format, target_sr, target_bits, target_channels
        ):
            print(
                f"  File already matches target format, "
                f"copying: {filename}"
            )
        ...
        # Transcode
        sr = wav_format['sample_rate']
        bits = wav_format['sample_width'] * 8
        ch = wav_format['channels']
        print(
            f"  Transcoding: {filename} "
            f"({sr}Hz, {bits}bit, {ch}ch)"
        )
        print(
            f"            -> "
            f"({target_sr}Hz, {target_bits}bit, {target_channels}ch)"
        )
...
def update_media_references(
    ...
    Returns:
        Dictionary mapping converted paths back to original paths
        (for restoration)
    """
    reverse_mapping = {}
    ...
    for track in timeline.tracks:
        ...
        for clip in track:
            ...
            ) or not clip.media_reference:
                continue
            if not hasattr(clip.media_reference, 'target_url'):
                continue

            original_url = clip.media_reference.target_url
            lookup_url = _normalize_file_url(original_url)
            ...
            if lookup_url in url_mapping:
                ...
                # Just store the plain path, let aaf_writer handle URL
...
def restore_media_references(
    ...
    for track in timeline.tracks:
        ...
        for clip in track:
            ...
            lookup_url = _normalize_file_url(current_url)

            if lookup_url in reverse_mapping:
                clip.media_reference.target_url = (
                    reverse_mapping[lookup_url]
                )
